...
class RemoteAgentConnections:
    ...
    async def send_message(self, message_request: SendMessageRequest) -> SendMessageResponse:
        logging.info(f"Sending message to agent '{self.card.name}': {message_request}")
        response = await self.agent_client.send_message(message_request)
        logging.info(f"Received response from agent '{self.card.name}': {response}")
        return response
...
class RoutingAgent:
    ...
    def list_remote_agents(self) -> str:
        ...
        for card in self.cards.values():
            logging.info(f"Agent Metadata - Name: {card.name}, Description: {card.description}, Skills: {card.skills}")
